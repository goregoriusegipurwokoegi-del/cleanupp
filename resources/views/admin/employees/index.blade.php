...
<div class="header-actions" style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
    ...
    <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
        <a href="{{ route('admin.employees.attendance') }}" style="background: var(--surface-variant); color: var(--text); text-decoration: none; padding: 0.8rem 1.5rem; border-radius: 12px; font-weight: 700; border: 1px solid var(--border-color); display: flex; align-items: center; gap: 0.5rem;">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
            Rekap Absensi
        </a>
        <button onclick="document.getElementById('add-employee-modal').style.display='flex'" style="background: var(--primary); color: #fff; border: none; padding: 0.8rem 1.5rem; border-radius: 12px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 0.5rem; transition: 0.3s; white-space: nowrap;">
    ...
    <table style="width: 100%; border-collapse: collapse; text-align: left; min-width: 700px;">
        ...
        <tbody>
            ...
            <tr style="border-bottom: 1px solid var(--border-color); transition: 0.3s;" onmouseover="this.style.background='var(--surface-variant)'" onmouseout="this.style.background='transparent'">
                ...
                <td style="padding: 1.5rem;">
                    <div style="display: flex; align-items: center; gap: 0.5rem; background: var(--surface-variant); border: 1px solid var(--border-color); padding: 0.4rem 0.8rem; border-radius: 8px; font-family: monospace; font-size: 0.85rem; width: fit-content;">
    ...
    <div class="glass-card" style="width: 90%; max-width: 450px; padding: 2.5rem; border-radius: 24px;">
        ...
        <form method="POST" action="{{ route('admin.employees.store') }}">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="text" name="name" required placeholder="Nama karyawan" style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none; transition: 0.3s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='var(--border-color)'" oninput="this.value = this.value.replace(/[0-9]/g, '');">>
            </div>

            <div style="margin-bottom: 1.5rem;">
                <label style="display: block; font-size: 0.85rem; margin-bottom: 0.5rem; opacity: 0.7;">Email Login</label>
                <input type="email" name="email" required placeholder="email@cleanup.com" style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none; transition: 0.3s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='var(--border-color)'">>
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 2rem;">
                <div>
                    <label style="display: block; font-size: 0.85rem; margin-bottom: 0.5rem; opacity: 0.7;">Jam Masuk</label>
                    <input type="time" name="work_start" required value="09:00" style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none;">
                </div>
                <div>
                    <label style="display: block; font-size: 0.85rem; margin-bottom: 0.5rem; opacity: 0.7;">Jam Pulang</label>
                    <input type="time" name="work_end" required value="17:00" style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none;">
                </div>
            </div>

            <div style="margin-bottom: 2rem;">
                <label style="display: block; font-size: 0.85rem; margin-bottom: 0.5rem; opacity: 0.7;">Password Default</label>
                <input type="text" name="password" required value="password123" style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none;">
            </div>

            <button type="submit" style="width: 100%; background: var(--primary); color: #fff; border: none; padding: 1rem; border-radius: 12px; font-weight: 700; cursor: pointer; transition: 0.3s;">Daftarkan Karyawan</button>
        </form>
    </div>
</div>
...
<div id="edit-employee-modal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.8); z-index: 1000; align-items: center; justify-content: center; backdrop-filter: blur(5px);">
    <div class="glass-card" style="width: 90%; max-width: 450px; padding: 2.5rem; border-radius: 24px;">
        ...
        <form id="edit-employee-form" method="POST">
            ...
            <div style="margin-bottom: 1.5rem;">
                <label style="display: block; font-size: 0.85rem; margin-bottom: 0.5rem; opacity: 0.7;">Nama Lengkap</label>
                <input type="text" name="name" id="edit-name" required style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none;" oninput="this.value = this.value.replace(/[0-9]/g, '');">>
            </div>

            <div style="margin-bottom: 1.5rem;">
                <label style="display: block; font-size: 0.85rem; margin-bottom: 0.5rem; opacity: 0.7;">Email Login</label>
                <input type="email" name="email" id="edit-email" required style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none;">
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                <div>
                    <label style="display: block; font-size: 0.85rem; margin-bottom: 0.5rem; opacity: 0.7;">Jam Masuk</label>
                    <input type="time" name="work_start" id="edit-work-start" required style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none;">
                </div>
                <div>
                    <label style="display: block; font-size: 0.85rem; margin-bottom: 0.5rem; opacity: 0.7;">Jam Pulang</label>
                    <input type="time" name="work_end" id="edit-work-end" required style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none;">
                </div>
            </div>
            ...
            <div style="margin-bottom: 2rem;">
                ...
                <input type="text" name="password" placeholder="Masukkan password baru" style="width: 100%; padding: 0.8rem 1.2rem; background: var(--surface-variant); border: 1px solid var(--border-color); border-radius: 12px; color: var(--text); outline: none;">
            </div>

            <button type="submit" style="width: 100%; background: var(--primary); color: #fff; border: none; padding: 1rem; border-radius: 12px; font-weight: 700; cursor: pointer;">Simpan Perubahan</button>
        </form>
    </div>
</div>
